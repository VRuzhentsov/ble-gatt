...
| `Draining` (Linux `pending_cleanup`) in this PR | **Deferred to a follow-up.** The `Draining` state exists in the machine; the Linux backend does not yet feed it. | `pending_cleanup` / `LinuxConnectGuard` shipped in #14 after ~6 automated-review P1 rounds and is proven only on hardware — this repo has no BlueZ CI, and Linux is not where the bug is (its `connect()` has no persistent "already open" refusal; BlueZ emits `Connected(false)` on adapter-off). Rewriting that path blind is the wrong risk. The follow-up does it with the peer's hardware in the loop. This PR: Linux emits `RadioChanged` and adopts `CentralLink`/`PeripheralLink` for the lifecycle it already models cleanly; the `dial_lock` → per-address change ships with the `Draining` follow-up. |
